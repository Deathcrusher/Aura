  ...
  <div className="p-6 rounded-2xl bg-white dark:bg-slate-800/70 border border-slate-200 dark:border-slate-700/50 shadow-lg w-full max-w-3xl animate-fade-in">
    <div className="flex items-center justify-between gap-3 mb-3">
      <div className="flex items-center gap-3">
        <LightbulbIcon className="w-6 h-6 text-[#6c2bee]"/>
      ...
      <div className="flex items-center gap-2">
        ...
        >
          <DownloadIcon className="w-6 h-6 text-[#6c2bee]" />
        </button>
        <button 
          onClick={onPlay}
        ...
        >
          {playbackState === 'loading' && <SpinnerIcon className="w-6 h-6 text-[#6c2bee]" />}
          {playbackState === 'playing' && <StopIcon className="w-6 h-6 text-[#6c2bee]" />}
          {playbackState === 'idle' && <PlayIcon className="w-6 h-6 text-[#6c2bee]" />}
        </button>
      </div>
    </div>
